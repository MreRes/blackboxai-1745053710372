async function processMessage(message, sender) {
    ...
    if (incomePattern.test(message)) {
        ...
        if (amount !== null) {
            ...
            response = `Pemasukan sebesar Rp ${amount} telah dicatat. Saldo Anda saat ini sebesar Rp ${report.balance}.`;
        } else {
        ...
        if (amount !== null) {
            ...
            const report = await getReport(userId);
            response = `Pengeluaran sebesar Rp ${amount} telah dicatat. Saldo Anda saat ini sebesar Rp ${report.balance}.`;
        } else {
