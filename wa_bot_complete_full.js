...
function startHttpServer() {
                ...
                try {
                    ...
                    res.end(`
                        ...
                        <body style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100vh; font-family:sans-serif;">
                            <h1>Scan this QR code with your WhatsApp mobile app</h1>
                            <img src="${qrDataUrl}" alt="WhatsApp QR Code" />
                            <p>If the QR code does not refresh, restart the bot.</p>
                        </body>
                        </html>
                    `);
                } catch (err) {
                    res.end('<p>Error generating QR code image.</p>');
                }
            } else {
                res.end('<p>Waiting for QR code...</p>');
            }
        } else {
            res.writeHead(404);
            res.end();
    ...
    server.listen(8000, () => {
        logger.info('QR code server running at http://localhost:8000');
    });
}
...
if (qr) {
    latestQr = qr;
    logger.info('QR code received, update available at http://localhost:8000');
    // Also print QR code in terminal for convenience with clear separators
    console.log('\n\n========== WHATSAPP QR CODE ==========');
    QRCode.toString(qr, { type: 'terminal' }, (err, url) => {
        if (err) {
            logger.error('Failed to generate QR code in terminal:', err);
        } else {
            console.log(url);
            ...
            if (connection === 'close') {
                ...
                if (shouldReconnect) {
                    setTimeout(() => {
                        startBot();
                    }, 5000); // wait 5 seconds before reconnecting to avoid rapid loops
                } else {
